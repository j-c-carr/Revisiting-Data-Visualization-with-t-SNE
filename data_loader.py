...
def load_data(dset_name, save_if_not_found=False):
    ...
    if not os.path.isfile(f'datasets/{dset_name}.pkl'):
    ...
    if not os.path.isfile(f'datasets/{dset_name}.pkl') and save_if_not_found:
        data = {'X': X, 'y': y}
        logging.debug(f'Saving dataset to datasets/{dset_name}.pkl...')
        with open(f'datasets/{dset_name}.pkl', 'wb') as f:
